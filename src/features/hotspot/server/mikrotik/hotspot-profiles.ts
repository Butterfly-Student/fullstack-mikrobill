...
const _HotspotProfilesSchema = z.object({
  routerId: z
    .number()
...
// Tipe TypeScript otomatis dari schema
type HotspotProfile = z.infer<typeof _HotspotProfilesSchema>

export const hotspotProfilesKeys = {
  all: ['hotspot-profiles'] as const,
  byRouter: (routerId: number) =>
    [...hotspotProfilesKeys.all, routerId] as const,
}

// Get all profiles
export const getHotspotProfiles = createServerFn()
  .validator((data: HotspotProfile) => data)
  .handler(async ({ data }) => {
