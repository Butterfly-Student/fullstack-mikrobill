...
export class MikrotikClient {
	 ...
	 */
	static setCachedDirectClient(config: IRosOptions, client: MikrotikClient): void {
	 ...
	 */
	static getCachedDirectClientByConfig(config: IRosOptions): MikrotikClient | null {
		const cacheKey = MikrotikClient.generateDirectCacheKey(config);
	 ...
	 */
	static async disconnectDirectClientByConfig(config: IRosOptions): Promise<void> {
		const cacheKey = MikrotikClient.generateDirectCacheKey(config);
		await MikrotikClient.disconnectCachedDirectClient(cacheKey);
	}
	...
	static getConnectionStats() {
		const stats = {
			totalConnections: MikrotikClient.clientCache.size + MikrotikClient.directClientCache.size,
			activeConnections: 0,
	 ...
	 */
	static async reconnectDirectClient(config: IRosOptions): Promise<MikrotikClient> {
	...
	static async cleanup(): Promise<void> {
		...
		await Promise.allSettled([...databaseDisconnectPromises, ...directDisconnectPromises]);
	...
	private static async cleanupStaleConnections(): Promise<void> {
		...
		if (staleRouterIds.length > 0 || staleDirectCacheKeys.length > 0) {
			...
			await Promise.allSettled([...databaseCleanupPromises, ...directCleanupPromises]);
		}
	}
