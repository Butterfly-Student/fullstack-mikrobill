...
export async function updateUser(
	id: string, // Changed parameter type
...
export async function assignRoleToUser(userId: string, roleName: string) {
	...
	if (role) {
		...
		await db.insert(schema.userRoles).values(userRole).onConflictDoNothing();
	}
}

export async function assignRolesToUser(userId: string, roleIds: number[]) {
	// Changed parameter type
	...
	if (roleIds.length > 0) {
		const userRoles: (typeof schema.userRoles.$inferInsert)[] = roleIds.map(
...
export async function hasPermission(
	userId: string, // Changed parameter type
	permission: string
): Promise<boolean> {
